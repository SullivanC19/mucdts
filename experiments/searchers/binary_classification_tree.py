...
class BinaryClassificationTree:
    ...
    def predict(self, X, N=1, P=1):
        if self.is_leaf():
            assert (self.label_counts is not None)
            pred_a = self.label_counts[1] * N > self.label_counts[0] * P
            pred_b = np.argmax([self.label_counts[0] / N, self.label_counts[1] / P])
            if pred_a != pred_b:
                print(self.label_counts, N, P, pred_a, pred_b)
                exit()
            return np.full(X.shape[0], self.label_counts[1] * N >= self.label_counts[0] * P)
        left, right = split(X, self.feature)
        y = np.zeros(X.shape[0], dtype=bool)
        y[left] = self.left.predict(X[left], N, P)
        y[right] = self.right.predict(X[right], N, P)
        return y
    ...
    def balanced_accuracy(self, X, y):
        num_neg = np.sum(y == 0)
        num_pos = np.sum(y == 1)
        y_pred = self.predict(X, N=num_neg, P=num_pos)
        print(np.sum(y_pred))
        return 0.5 * (np.sum(y_pred[y == 0] == 0) / num_neg + \
                      np.sum(y_pred[y == 1] == 1) / num_pos)
